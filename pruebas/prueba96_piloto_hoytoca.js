...
c(/function selectorModo\(\)/.test(K) && /Piloto automático/.test(K) && /Mando manual/.test(K) && (K.match(/selectorModo\(\)/g) || []).length >= 3, "   con su selector en «Mis grupos» y dentro de cada grupo");
...
c(/'<div class="gp-herr" data-av>'/.test(K), "🔴 «Para todos tus grupos» y «Para tus Geniallys», solo en mando manual");
...
c(/class="card pt-msg" data-av/.test(K) && /id="pt-panel-ed" data-av/.test(K) && /data-av data-escribir=/.test(K), "   en la portada: el mensaje a tus reclutas, cambiar el panel y «Escribirles», en mando manual");
...
c(/<details class="gp-hoy-d"/.test(K) && /bloqueHoyToca\(S, sem, Number\(p\.total\) \|\| 15, null\)/.test(K), "🔴 en «Mis grupos», desplegado (abierto si solo hay un grupo en marcha)");
c(/'<div class="card pt-hoy">' \+ bloqueHoyToca\(\(DATOS\.proyecto \|\| \{\}\)\.stargate \|\| \{\}, sem, total, gente\)/.test(K), "   y en la portada, con cuántos lo han hecho");
