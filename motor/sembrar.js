...
async function main() {
  ...
  for (const [col, lista] of [["missions", paq.misiones], ["campaigns", paq.campanas], ["rewards", paq.recompensas]]) {
    for (let i = 0; i < lista.length; i += 400) {
      ...
      lista.slice(i, i + 400).forEach(x => {
        lote.set(db.collection(col).doc(id + "__" + x.id), Object.assign({}, x, { projectId: id, stargateId: x.id }));
      });
      await lote.commit();
    }
  ...
  if (conReclutas) {
    ...
    for (let k = 0; k < SIEMBRA.length; k++) {
      const r = SIEMBRA[k];
      const hechas = ["H1"].concat(r.retos);
      ...
      hechas.forEach((rid, j) => {
        ...
        const m = porId[rid]; if (m) { xp0.xp += m.points || 0; xp0.cred += m.coinsReward || 0; }
      });
