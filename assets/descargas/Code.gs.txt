...
function onOpen() {
  SpreadsheetApp.getUi().createMenu("🛰️ STARGATE")
...
function asegurarHojaPERs_() {
  ...
  if (!sh) {
    ...
    sh.appendRow(["id","PER","Profesorado","Apertura","Cierre","Estado","Formulario (editar)","Formulario (alumnado)","Hoja de respuestas","Tablero web","Creado"]);
    sh.setFrozenRows(1);
  }
  return sh;
}
...
function crearPER(datos) {
  ...
  var sub = carpeta.createFolder("PER " + nombre);

  // hoja de respuestas
  var ss = SpreadsheetApp.create("STARGATE · " + nombre + " · respuestas");
  DriveApp.getFileById(ss.getId()).moveTo(sub);
  ...
  sh.appendRow([id, nombre, datos.profesores || "", apertura || "", cierre || "", estado,
                form.getEditUrl(), form.getPublishedUrl(), ss.getUrl(), tablero, new Date()]);
  return { id: id, nombre: nombre, formAlumnado: form.getPublishedUrl(), formEditar: form.getEditUrl(),
           hoja: ss.getUrl(), tablero: tablero, estado: estado, web: "https://stargate.mistercuarter.es/registro.html?per=" + id,
...
function tablero_(perId) {
  ...
  var ss = SpreadsheetApp.openByUrl(fila[8]);
  var hoja = ss.getSheets()[0]; var vals = hoja.getDataRange().getValues();
