...
function crearPER(datos) {
  ...
  reestructurarCanje_(fc, { inicio: datos.inicio, tipo: tipo });   // pone las preguntas, las secciones y el enrutado, todo en un sitio
  publicar_(fc);
  _creados.push(fc);
  ...
  // 🔴 9-sep · PRIMERO LO QUE SE REPARTE, DESPUES LO BONITO. Lo pregunto Norberto («si falla al
  // principio estamos muertos») y tenia razon el orden estaba al reves: iban las ~23 imagenes de
  // los formularios ANTES del documento de enlaces. O sea que en el peor caso el profe se quedaba
  // con los formularios ilustrados y SIN el documento, que es lo unico que tiene que repartir a su
  // clase. Ahora: documento -> dossier -> imagenes. Si algo se cae, lo que falta es cosmetica, y la
  // continuacion la pone dentro de un minuto sin que nadie haga nada.
  var docUrl = "";
  if (_t.sobra(60000)) { try { docUrl = crearDocumentoPER_(id); pend.doc = false; _t.hito("documento de enlaces"); }
                         catch (e) { Logger.log("crearDocumentoPER_: " + e); } }
  var dossierUrl = "";
  if (_t.sobra(45000)) { try { dossierUrl = dossier_(); pend.dossier = false; _t.hito("dossier del profesorado"); }
                         catch (e) { Logger.log("dossier_: " + e); } }
  if (_t.sobra(30000)) { try { var _im = imagenesFormularios_(fb, ft, fc, _t); pend.imagenes = _im.faltan > 0; _t.hito("imagenes de los formularios (" + _im.puestos + " puestas, " + _im.faltan + " pendientes)"); }
...
// 🔴 11-sep · «Un dato más: la actividad» no decía nada. Ahora la sección tiene nombre propio y
// avisa de la ventana: es LA pieza que evita que setenta personas con un 9,5 compren medio punto en
// la semana 3. El aviso se escribe con la FECHA EXACTA del grupo, no con «la semana 15» — nadie
// sabe de memoria qué día es la semana 15.
var SEC_NOTA   = "⚔️ Arsenal de Batalla";
...
// La fecha en que se abre el Arsenal para ESTE grupo. Sin PER (o sin fecha de inicio) se dice la
// semana a secas: mejor una frase honesta que una fecha inventada.
function aperturaArsenal_(o) {
  var sem = desdeEfectiva_(SEMANA_ARSENAL, o && o.tipo);
  if (!o || !o.inicio) return { sem: sem, fecha: "" };
  var d = new Date(String(o.inicio) + "T00:00:00");
  if (isNaN(d.getTime())) return { sem: sem, fecha: "" };
  d.setDate(d.getDate() + (sem - 1) * 7);
  return { sem: sem, fecha: Utilities.formatDate(d, "Europe/Madrid", "d 'de' MMMM 'de' yyyy") };
}
function ayudaSeccion_(sec, o) {
  if (sec !== SEC_NOTA) return AYUDA_SECCION[sec] || "";
  var a = aperturaArsenal_(o);
  return "🔴 EL ARSENAL SOLO SE ABRE AL FINAL: a partir de la semana " + a.sem +
    (a.fecha ? " (" + a.fecha + ")" : "") + ". Antes de esa fecha esta solicitud se DENIEGA y no se " +
    "te cobra nada.\n\n" +
    "Y antes de gastar: si ya tienes la nota máxima de evaluación continua, subirla no te da nada. " +
    "Mira tu nota primero. Si no la necesitas, esos créditos rinden muchísimo más en el álbum de " +
    "cromos o en el vestuario de héroes.\n\n" +
    "Dime en qué entregable quieres que se aplique.";
}
...
// 🔴 SOLO CUENTAN LOS TITULOS QUE EXISTEN. El destino de cada item es su posicion en la lista, asi
// que un titulo que todavia no esta en el formulario deja un HUECO que corre a todos los demas: el
// orden pedido se vuelve inalcanzable y cada pasada reintenta los mismos movimientos, para siempre.
// Visto en produccion el 30-ago con el canje: la lista pedia las imagenes de las diez recompensas
// —que se suben en otro paso— y DOS pasadas de 6 minutos dejaron el formulario EXACTAMENTE IGUAL.
function ordenarItems_(form, titulos) {
  var hay = {};
  form.getItems().forEach(function(i){ hay[i.getTitle()] = true; });
  titulos.filter(function(t){ return hay[t]; }).forEach(function(t, destino){
...
// `o` es el PER, opcional: sirve para escribir la fecha exacta del Arsenal. Sin el, el texto dice
// la semana a secas y sigue siendo cierto.
function reestructurarCanje_(fc, o) {
  var _perCanje = o || null;
  ...
  cambioAyuda_(act, ayudaSeccion_(SEC_NOTA, _perCanje));
  ...
  SECCIONES_CANJE.forEach(function(sec){
    if (cat.some(function(x){ return seccionDe_(x.tipo) === sec; })) quiero.push({ titulo: sec, ayuda: ayudaSeccion_(sec, _perCanje) });
  });
...
function actualizarRecompensas() {
  ...
  ui.alert(r.atascado ? "Actualización DETENIDA" : r.terminado ? "Formularios actualizados" : "Actualización en marcha (va por lotes)",
    (r.atascado
      ? "Me he parado solo tras varios intentos sin avanzar, para no quedarme dando vueltas y " +
        "gastar cuota. Abajo tienes dónde se atascó. Lo demás SÍ está aplicado."
      : r.terminado
...
function actualizarFormularios_() {
  ...
  // 7 minutos: más que el corte duro. Si la pasada acaba bien, al final se re-arma a 1 minuto; si
  // muere en seco, este salta y la retoma. Nunca dos pasadas a la vez.
  programarContinuacion_("continuarActualizarFormularios", 420000);
  var apuntar = function(){ try { guardarProgreso_("formularios", pr); } catch (e) {} };
  while (pr.i < pers.length && t.puedo()) {
    ...
    if (pr.fase === 0) {
      ...
      try { var f = formDelPER_(perObj_(v), "C"); if (!f) throw new Error("sin formulario de canje");
            ...
            reestructurarCanje_(f, perObj_(v)); pr.canjes++; }
      catch (e) { pr.fallos.push(String(v[1]) + " (canje): " + e.message); }
      pr.fase = 1; hecho = true; t.marcar(); apuntar();
      if (!t.puedo()) break;
    }
    if (pr.fase === 1) {
    ...
    if (pr.fase === 2) {
      ...
      pr.fase = 3; hecho = true; t.marcar(); apuntar();
      if (!t.puedo()) break;
    }
    // 🔴 fase 3 · LAS IMAGENES. Sin esto solo las tendrian los PER recien creados, y los grupos que
    // ya existen se quedarian con el canje mudo para siempre. Va en su propia fase porque son ~30
    // descargas: es justo lo que no cabe junto a lo demas en los 6 minutos.
    if (!cabe()) break;
    // el reloj tambien aqui: lo que no quepa se anota como «faltan» y se completa en otra pasada
    // (las imagenes ya puestas se saltan solas), en vez de morir a mitad y dejar la tanda colgada.
    try { var im = imagenesFormularios_(formDelPER_(perObj_(v), "B"), formDelPER_(perObj_(v), "T"), formDelPER_(perObj_(v), "C"), t);
          if (im.faltan) pr.fallos.push(String(v[1]) + " (imagenes): faltan " + im.faltan); }
    catch (e) { pr.fallos.push(String(v[1]) + " (imagenes): " + e.message); }
    pr.i++; pr.n++; pr.fase = 0; hecho = true; t.marcar(); apuntar();
  }
  ...
  else { apuntar(); programarContinuacion_("continuarActualizarFormularios", 60000); }
...
// v3.61 · Un PER es «de practicas» si lleva DEMO en el nombre. Este grupo se le ofrece a TODO el
// profesorado, asi que la regla tiene que ser estrecha.
// 🔴 Empece aceptando tambien PRUEBA, por reutilizar la de la Nave (recluta.js, demoPermitido). La
// bateria 49 enseño el problema: los grupos desechables del referente —«PRUEBA V54 DOS
// REFERENTES» y compania— apareceran en la sala de todos los docentes como si fueran para ellos.
// La Nave usa PRUEBA para otra cosa (permitir el modo demostracion en un grupo cualquiera), y eso
// no obliga a que aqui signifique lo mismo.
function esDemo_(o) { return /\bDEMO\b/i.test(String((o && o.nombre) || "")); }
...
function construirTicket_(ft, referente, profesores, perId) {
  ...
  for (var t = 1; t <= 8; t++) {
    ...
    // 23-sep · sin las dos actividades (Norberto: «solo presentación, 8 temas y final»)
  }
...
function continuarAltaPER() {
  ...
  // el MISMO orden que en el alta: primero lo que se reparte, al final lo bonito.
  ...
  if (pr.imagenes && t.puedo()) {
    // 🔴 aqui TAMBIEN va el reloj. Sin el, la continuacion podia comerse sus propios 6 minutos en
    // las imagenes y morir antes del documento — el mismo fallo que aplaza, repetido en el que
    // deberia rematarlo: el PER no se terminaba NUNCA por muchas vueltas que diera.
    try { var im = imagenesFormularios_(formDelPER_(o, "B"), formDelPER_(o, "T"), formDelPER_(o, "C"), t);
...
// v3.65 · ¿Se exige entrar con Google para ver la ficha? Interruptor, no decision grabada a fuego:
// se enciende poniendo EXIGE_LOGIN a "si" en las propiedades del script, y se apaga quitandolo.
// Apagado mientras el login sea una prueba: si fallara, nadie se queda sin ver su Nave — y los
// formularios, que es donde se trabaja de verdad, no dependen de esto.
function exigeLogin_() {
  return (PropertiesService.getScriptProperties().getProperty("EXIGE_LOGIN") || "").toLowerCase() === "si";
}
...
function concedidasDe_(sh, fila, email, nombreRec) {
  ...
  for (var i = 1; i < v.length; i++) {
    ...
    // 🔴 11-sep · LAS PENDIENTES TAMBIÉN CUENTAN. Si no, alguien manda cinco solicitudes iguales de
    // «Subir 1 punto» y le llena la lista al docente de duplicados — justo el goteo que la cola
    // venía a evitar. Una pendiente ocupa el sitio hasta que se resuelve; si se rechaza, deja de
    // contar y puede volver a pedirla.
    var est = String(v[i][cE] || "");
    if (est.indexOf("Concedido") === 0 || est.indexOf(EST_PENDIENTE) === 0) n++;
  }
  return n;
}
...
function resolverCanje_(o, sh, fila) {
  ...
  // 2) tope por alumno: el catálogo dice cuántas veces puede concederse a la misma persona
    ...
    // si lo que ocupa el sitio es una solicitud sin resolver, decirlo: no es lo mismo «ya lo tienes»
    // que «lo has pedido y tu profesorado aún no ha contestado»
    var _pend = ficha.tipo === "nota";
    cuerpo = ficha.max === 1
  ...
  // 5) recompensas de nota: NO se conceden solas. Quedan PENDIENTES y el profesorado las resuelve
  // en bloque desde su panel.
  // 🔴 11-sep · Norberto: «cambiar 100 notas con cada actividad es una carga de trabajo INMENSA y no
  // compensa». El problema no era solo cuántas llegan —eso lo arreglan la semana 15 y los precios—
  // sino CÓMO llegan: concediéndose solas, una por una, con un correo cada vez. Eso es trabajo a
  // goteo y sin decidir nada. Ahora se acumulan en una lista y se resuelven de una sentada.
  // Los créditos NO se cobran al pedirlo: `gastado` solo suma las filas que empiezan por
  // «Concedido», así que una solicitud pendiente no cuesta nada hasta que se aprueba. Si se rechaza,
  // no se ha movido un crédito y no hay nada que devolver.
  else {
    estado = EST_PENDIENTE;
    cuerpo = "Solicitud registrada: " + rec + ". 🔴 Todavía NO se te han cobrado los créditos y la " +
      "recompensa NO está concedida: la revisa tu profesorado y te llegará su respuesta. " +
      "Si ya tienes la nota máxima de evaluación continua, avísale para retirarla — así no gastas " +
      "los créditos en algo que no te sube nada.";
  }
  var ok = estado === "Concedido";
  var pendiente = estado === EST_PENDIENTE;
  ...
  if (pendiente && ficha && ficha.tipo === "nota") {
...
function actualizarConsola() {
  ...
  pers.forEach(function(v){
    ...
    // 11-sep · «pendientes» pasa a significar SOLICITUDES SIN RESOLVER, que es lo que ahora le hace
    // falta ver al docente de un vistazo: cuántas le esperan en la cola.
    var pendientes = canjes.filter(function(c){ return c.estado.indexOf(EST_PENDIENTE) === 0; });
...
// 🔴 11-sep · EL PARTE DE SALUD SE MUDÓ A Bonus.gs. Code.gs volvió a rozar el techo de Apps
// Script (299.714 bytes de copia ASCII, tope medido 298.000) y la regla del proyecto es sacar un
// bloque, no pelearse con el botón de guardar. El parte era el candidato limpio: es una pieza
// entera y autónoma —solo informa, no repara nada— y los documentos de instalación ya decían que
// vivía ahí. Es la CUARTA vez que pasa: la próxima, otro bloque.
...
function doGet(e) {
  ...
  if (e && e.parameter && e.parameter.accion === "verificar_token") {
    ...
    try { res.correo = correoDeToken_(e.parameter.token); res.ok = true; }
...
function doPost(e) {
  ...
  try {
    ...
    if (q.accion === "quien") {
      ...
      // 🔴 La comprobacion tiene que estar AQUI. Si el navegador verificara el token y luego pidiera
      // la ficha por correo, el troll se saltaria el primer paso y pediria la ficha directamente.
      var eq;
      if (q.token) eq = correoDeToken_(q.token);
      else if (exigeLogin_()) throw new Error("Entra con tu cuenta de Google para ver tu ficha.");
      else eq = String(q.email || "").toLowerCase().trim();
      ...
      return ContentService.createTextOutput(JSON.stringify({ yo: yo ? { alias:yo.alias, avatar:yo.avatar, xp:yo.xp,
        ...
        xp7:yo.xp7 || 0,
        repes:yo.repes || 0, repes_disponibles:yo.repes_disponibles || 0, racha:yo.racha || 0 } : null, correo: eq,
    ...
    // v3.61 · QUIEN ERES, POR CORREO. Antes la sala pintaba un desplegable con los NOMBRES de todo
    // el profesorado y cada uno elegia el suyo: cualquiera con el PIN veia la plantilla entera y
    // podia entrar como un companero. Ahora el docente escribe su correo y el SERVIDOR decide que
    // grupos le tocan. El navegador sigue sin recibir ni un correo ajeno.
    // 🔴 Esto es ORDEN, no seguridad: quien tenga el PIN y sepa el correo de otro puede escribirlo.
    // La puerta de verdad sigue siendo el PIN.
    if (a === "pers") {
      var _correo = String(q.correo || "").trim().toLowerCase();
      var _todos = hoja_(H.PERS).getDataRange().getValues().slice(1).filter(function(v){ return v[0]; })
        .map(function(v){ var ob = perObj_(v);
          var _docs = docentesDe_(ob.id);
          ob.docentes = _docs.map(function(d){ return { nombre:d.nombre, rol:d.rol, panel:d.panel }; });
          ob.demo = esDemo_(ob);
          ob._mio = !!_correo && _docs.some(function(d){ return d.correo === _correo; });
          ob._yo = ob._mio ? (_docs.filter(function(d){ return d.correo === _correo; })[0] || {}).nombre || "" : "";
          ob.semana = semanaDe_(ob); return ob; });
      if (!_correo) { _todos.forEach(function(o){ delete o._mio; delete o._yo; }); out = { pers: _todos }; }
      else {
        var _mios = _todos.filter(function(o){ return o._mio; });
        // Sin grupo asignado no se enseña la sala vacia: se enseña la DEMO, que es donde se practica
        // sin miedo. Y quien si tiene grupos tambien la conserva, por lo mismo.
        var _demo = _todos.filter(function(o){ return o.demo && !o._mio; });
        var _nombre = (_mios.filter(function(o){ return o._yo; })[0] || {})._yo || "";
        out = { pers: _mios.concat(_demo), yo: { correo:_correo, nombre:_nombre, encontrado: _mios.length > 0 },
                demo: _demo.map(function(o){ return o.id; }) };
      }
    }
    ...
    // 11-sep · la cola de solicitudes de nota: leerlas y resolverlas en bloque
    else if (a === "pendientes") { out = pendientesNota_(per); }
    else if (a === "pendiente_resolver") {
      out = resolverPendiente_(per, q.fila, q.profe, q.aprueba === true || q.aprueba === "true", q.motivo);
    }
...
// ================= UTILIDADES =================
// v3.65 · IDENTIDAD VERIFICADA POR GOOGLE. Devuelve el correo de quien pulso «Iniciar sesion con
// Google», o revienta. 🔴 Las tres comprobaciones son las tres: que Google reconozca el token, que
// se emitiera PARA NOSOTROS (`aud`) y que el correo este verificado. Sin la de `aud`, el token de
// cualquier otra web valdria para entrar como quien se quisiera. Probado con un token falsificado
// que decia ser nuestro: rechazado.
function correoDeToken_(token) {
  ...
  var r = UrlFetchApp.fetch("https://oauth2.googleapis.com/tokeninfo?id_token=" + encodeURIComponent(tok),
