...
(function(){
  ...
  get(API+'?per='+encodeURIComponent(per),function(d){
    ...
    var filas=r.map(function(p){return '<tr data-alias="'+esc(p.alias.toLowerCase())+'"><td><b>'+p.pos+'</b></td><td class="who">'+SG.avatarImg(p.avatar,p.alias,p.marco==='oro'?'marco-oro':'')+'<span><b>'+(p.corona?'👑 ':'')+esc(p.alias)+'</b>'+(p.titulo?'<em class="titulo-recluta">«'+esc(p.titulo)+'»</em>':'')+'</span></td><td>'+esc(p.planeta)+'</td><td>'+p.n+'/24</td><td class="small"><b>'+(SG.nivel?SG.nivel(p.xp,d.tipo):1)+'</b> <span class="muted">'+esc(SG.avatarSrc(p.avatar,p.alias,p.xp,d.tipo).rango)+'</span></td><td class="pts">'+p.xp+'</td><td class="small muted">'+(p.creditos!=null?p.creditos:(p.xp_disponibles||0))+'</td></tr><tr class="insrow" data-alias="'+esc(p.alias.toLowerCase())+'"><td colspan="6"><div class="dots">'+dots(p)+'</div></td></tr>';}).join('');
    root.innerHTML='<div class="tab-head"><div><div class="eyebrow amber">'+esc(d.nombre)+' · '+esc(d.tipo)+' · '+esc(d.estado)+'</div><h3>Ranking de reclutas</h3></div><div class="small muted">'+r.length+' reclutas · '+new Date(d.actualizado).toLocaleString('es-ES')+'</div></div>'
      ...
      +(r.length?'<div class="tablewrap"><table class="rank"><thead><tr><th>#</th><th>Recluta</th><th>Planeta</th><th>Insignias</th><th>Nivel</th><th>xp</th><th>◈</th></tr></thead><tbody id="rankBody">'+filas+'</tbody></table></div>':'<p class="lead">Todavía nadie se ha alistado. Sé el primero: rellena tu Bitácora de mando.</p>');
