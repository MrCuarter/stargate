...
(function(){
  ...
  get(API+'?per='+encodeURIComponent(per),function(d){
    ...
    var podio=top.length?'<div class="podium">'+[1,0,2].map(function(i){var p=top[i];if(!p)return '';var cls=['gold','silver','bronze'][i],med=['🥇','🥈','🥉'][i];return '<div class="pod '+cls+'"><div class="medal">'+med+'</div>'+SG.avatarImg(p.avatar,p.alias,'big',p.xp,d.tipo)+'<div><b>'+esc(p.alias)+'</b></div><div class="rango">'+SG.avatarSrc(p.avatar,p.alias,p.xp,d.tipo).rango+'</div><div class="pts">'+p.xp+' xp</div><div class="h"></div></div>';}).join('')+'</div>':'';
    var filas=r.map(function(p){return '<tr data-alias="'+esc(p.alias.toLowerCase())+'"><td><b>'+p.pos+'</b></td><td class="who">'+SG.avatarImg(p.avatar,p.alias,'')+'<b>'+esc(p.alias)+'</b></td><td>'+esc(p.planeta)+'</td><td>'+p.n+'/24</td><td class="pts">'+p.xp+'</td><td class="small muted">'+p.xp_disponibles+'</td></tr><tr class="insrow" data-alias="'+esc(p.alias.toLowerCase())+'"><td colspan="6"><div class="dots">'+dots(p)+'</div></td></tr>';}).join('');
