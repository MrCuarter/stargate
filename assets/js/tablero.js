...
(function(){
  ...
  get(API+'?per='+encodeURIComponent(per),function(d){
    ...
    var podio=top.length?'<div class="podium">'+[1,0,2].map(function(i){var p=top[i];if(!p)return '';var cls=['gold','silver','bronze'][i],med=['🥇','🥈','🥉'][i];return '<div class="pod '+cls+'"><div class="medal">'+med+'</div>'+SG.avatarImg(p.avatar,p.alias,'big')+'<div><b>'+esc(p.alias)+'</b></div><div class="pts">'+p.xp+' xp</div><div class="h"></div></div>';}).join('')+'</div>':'';
