...
(function(){
  ...
  if(!API){root.innerHTML='<div class="wip"><span class="ic"><img class=ico src=assets/img/iconos/p/envivo.png alt></span><div><b>Panel pendiente de conectar</b> (falta la URL del web app).</div></div>';return;}
  ...
  function sel(){return '<select id="selPer">'+st.pers.map(function(p){return '<option value="'+esc(p.id)+'"'+(p.id===st.per?' selected':'')+'>'+esc(p.nombre)+' · '+esc(p.tipo)+' · '+(p.archivado?'archivado':esc(p.estado))+'</option>';}).join('')+'</select>';}
  function render(){var d=st.datos,p=st.pers.filter(function(x){return x.id===st.per;})[0]||{};
    root.innerHTML='<div class="tab-head"><div><div class="eyebrow amber">Panel del profesorado</div><h3>'+esc(d.nombre)+'</h3><div class="small muted">'+esc(d.tipo)+' · '+esc(d.estado)+' · referente: '+esc(referentes(d))+' · profesorado: '+esc(d.profesorado||'—')+' · semana 1: '+esc(d.inicio||'sin fecha')+(d.cierre_canje?' · misiones hasta '+fecha(d.cierre_misiones)+' · canje hasta '+fecha(d.cierre_canje):'')+(d.archivado?' · <b><img class=ico src=assets/img/iconos/p/botin.png alt> ARCHIVADO</b> ('+esc(d.archivado)+')':'')+'</div></div><div>'+sel()+'</div></div>'
      +'<div class="tabs"><button data-v="alumnos"><img class=ico src=assets/img/iconos/p/gente.png alt> Alumnos ('+(d.reclutas||[]).length+')</button><button data-v="tickets"><img class=ico src=assets/img/iconos/p/ticket.png alt> Tickets de salida</button><button data-v="canjes"><img class=ico src=assets/img/iconos/p/premios.png alt> Canjes</button>'+(esReferente()?'<button data-v="per"><img class=ico src=assets/img/iconos/p/ajustes.png alt> Ajustes del PER</button>':'')+'<button class="salir">Salir</button></div><div id="vista"></div>';
  ...
  function vAlumnos(v){var r=st.datos.reclutas||[];var retos=RET[st.datos.tipo]||[];
    ...
    var alerta=(sinDoc.length||sinMail.length)
      ? '<div class="card" style="border-color:#f5b043;margin-bottom:14px"><h3><img class=ico src=assets/img/iconos/p/aviso.png alt> Avisos que se quedarían sin destinatario</h3>'
        ...
        +(sinMail.length?'<p class="small"><b>Docente(s) sin correo:</b> '+sinMail.map(esc).join(', ')
          +'. Sin correo no reciben ningún aviso. Ponlo en <b><img class=ico src=assets/img/iconos/p/ajustes.png alt> Ajustes del PER</b>.</p>':'')
        +'</div>'
      : '';
  ...
  function vPer(v){var d=st.datos;var docs=(d.docentes_full||[]).slice();if(!docs.length)docs=[{nombre:d.referente||'',correo:'',rol:'referente+imparte'}];
    v.innerHTML='<div class="grid cols-2"><div class="card"><h3>Equipo docente</h3>'
      ...
      +'<div class="card doc-card" style="grid-column:1/-1"><h3><img class=ico src=assets/img/iconos/p/notas.png alt> Todo el PER en un documento</h3>'
      ...
      +'<div class="card" style="grid-column:1/-1"><h3><img class=ico src=assets/img/iconos/p/varios.png alt> Panel de control (Genially de los planetas)</h3><p class="small muted">'+(d.panelPropio?'Este PER usa un panel <b>propio</b>.':'Este PER usa el panel <b>estándar</b> compartido. Si quieres usar una copia tuya, pega aquí sus enlaces (vacía los dos campos para volver al estándar).')+'</p>'
