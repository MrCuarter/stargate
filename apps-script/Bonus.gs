...
/**
 * Crea el ticket de salida ÚNICO: uno para todos los grupos y para siempre.
 *
 * Sustituye a «Crear PER» en lo único que quedaba por hacer aquí. Se ejecuta UNA VEZ en la vida:
 * crea el formulario, lo engancha a una hoja nueva dedicada solo a esto, y escribe en pantalla la
 * dirección con los huecos ya preparados para que la Nave los rellene sola.
 *
 * 🔴 A partir de ahí, crear un grupo nuevo no requiere tocar nada de esto. Es la misma idea que los
 * enlaces de validación de los Geniallys: el enlace no lleva el grupo, lo pone quien entra.
 */
function crearTicketUnico() {
  ...
  if (_yo && _dueno && _yo !== _dueno) {
    ui.alert("Estás con la cuenta " + _yo + " y la hoja es de " + _dueno + ".\n\n" +
      "El formulario y la hoja nueva quedarían en propiedad de " + _yo + " y no de la universidad, " +
      "y después no se pueden transferir sin que el destinatario lo acepte.\n\nEntra con " +
      _dueno + " y repítelo.");
    return;
  }
  var P = PropertiesService.getScriptProperties();
  var ya = P.getProperty("TICKET_UNICO_URL");
  if (ya) {
    var r = ui.alert("Ya existe un ticket compartido", "Su dirección es:\n\n" + ya +
      "\n\n¿Crear OTRO? (solo si el anterior se ha borrado)", ui.ButtonSet.YES_NO);
  ...
  // Hoja propia: la de mando se queda como está y esta solo recoge tickets, de todos los grupos y
  // de todos los años. Separarlas es lo que permite congelar la vieja el día que toque.
  var hojaT = SpreadsheetApp.create("STARGATE · Tickets de salida");
  try { DriveApp.getFileById(hojaT.getId()).moveTo(raiz); } catch (e) {}

  var ft = formDesdePlantilla_("PLANTILLA · Ticket de salida",
    "STARGATE · Contacta con NEBULA (ticket de salida)", raiz);
  construirTicket_(ft, "", "", "*");          // "*" = el compartido: grupo y docente son texto
  publicar_(ft);
  try { vincular_(ft, hojaT.getId()); } catch (e) {
    ui.alert("Se ha creado el formulario pero no he podido engancharlo a la hoja: " + e.message);
  }

  // 🔴 La dirección con huecos NO se escribe a mano. Los identificadores de campo de un formulario
  // («entry.1234567») no son los de sus preguntas y no hay forma de deducirlos: quien los escriba a
  // ojo se equivoca y el hueco no se rellena, sin decir nada. Google los da si le pides una
  // respuesta rellenada, así que se le pide una con dos palabras centinela y se sustituyen después.
  var base = ft.getPublishedUrl(), plantilla = base;
  try {
    var r = ft.createResponse(), puestos = 0;
    ft.getItems(FormApp.ItemType.TEXT).forEach(function (it) {
      var t = it.getTitle();
      if (t === TIT_GRUPO_TICKET) { r.withItemResponse(it.asTextItem().createResponse("XGRUPOX")); puestos++; }
      else if (t.indexOf("imparte tu clase") >= 0) { r.withItemResponse(it.asTextItem().createResponse("XMANDOX")); puestos++; }
    });
    if (puestos) plantilla = r.toPrefilledUrl().replace("XGRUPOX", "{GRUPO}").replace("XMANDOX", "{COMANDANTE}");
  } catch (e) { Logger.log("prefill: " + e); }

  P.setProperty("TICKET_UNICO_URL", base);
  P.setProperty("TICKET_UNICO_PLANTILLA", plantilla);
  P.setProperty("TICKET_UNICO_HOJA", hojaT.getId());

  ui.alert("Ticket compartido creado",
    "FORMULARIO\n" + base +
    "\n\nDIRECCIÓN CON HUECOS (esto es lo que hay que poner en la web)\n" + plantilla +
    "\n\nHOJA DE RESPUESTAS\n" + hojaT.getUrl() +
    "\n\nNo hay que repetir esto nunca más: vale para todos los grupos y todos los años.",
    ui.ButtonSet.OK);
}
...
function verTicketUnico() {
  var P = PropertiesService.getScriptProperties();
  var base = P.getProperty("TICKET_UNICO_URL");
  if (!base) { SpreadsheetApp.getUi().alert("Todavía no se ha creado el ticket compartido."); return; }
  SpreadsheetApp.getUi().alert("Ticket compartido",
    "FORMULARIO\n" + base + "\n\nDIRECCIÓN CON HUECOS\n" +
    (P.getProperty("TICKET_UNICO_PLANTILLA") || "—") + "\n\nHOJA DE RESPUESTAS\n" +
    "https://docs.google.com/spreadsheets/d/" + (P.getProperty("TICKET_UNICO_HOJA") || "") + "/edit",
