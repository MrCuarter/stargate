...
AVERIAS = [
 ...
 ("retos", "A un estudiante no le suman los retos o los créditos",
  ...
  "Dos cosas lo explican casi siempre: 1) casi todos piden el <b>enlace</b> de lo que se ha hecho "
  "(el secreto se registra al salir del Escape UNI): sin lo que piden, no se registran; 2) lo que toca la nota espera en la <b>Cola de nota</b> "
...
<p style="margin-top:12px"><a class="btn primary" href="recluta.html?per={PER_DEMO}&amp;demo=1" target="_blank" rel="noopener"><img class=ico src=assets/img/iconos/p/cohete.png alt> Probar la Nave como estudiante ↗</a></p>
...
<p style="margin-top:18px"><span class="pill">8 planetas = 8 temas</span><span class="pill">{len(NIVELES)} niveles</span><span class="pill">{sum(1 for c in CROMOS)} cromos</span>{('<span class="pill">' + str(TOPE_RETOS_SEMANA) + ' retos por semana, como mucho</span>') if TOPE_RETOS_SEMANA else ''}</p>
</header>
...
open(os.path.join(HERE,"assets","js","tour.js"),"w",encoding="utf-8").write(
  TOUR_JS.replace("__CRED_A__", str(CREDITOS["relampago"])).replace("__CRED_B__", str(CREDITOS["retoB"]))
         .replace("__TOPE_FRASE__", ("Nadie registra más de %d retos por semana." % TOPE_RETOS_SEMANA) if TOPE_RETOS_SEMANA else "Sin tope: a su ritmo, y si alguien registra muchos de golpe, su docente lo ve."))
...
bloqueado… o mejor aún, que aparezca el vídeo dos semanas más tarde: el del tema 6 aparecería en el tema 7, el último
día. Así todos lo verán, pero los que completen la misión lo verán antes».
...
"""
def _tema_n(tema):
    m = __import__("re").search(r"Tema (\d)", str(tema or ""))
    return int(m.group(1)) if m else 0

def _fragmentos():
    import re as _re
    # en qué semana acaba cada tema
    fin_tema = {}
    for s in CRONO:
        n = _tema_n(s["tema"])
        if n: fin_tema[n] = max(fin_tema.get(n, 0), s["sem"])
    out = []
    for s in CRONO:
        for c, cuando in s["videos"]:
            v = yt(c)
            m = _re.match(r"Fragmento\s*(\d+)\s*·\s*(.+)$", v["titulo"].strip())
            if not m: continue
            n, quien = int(m.group(1)), m.group(2).strip()
            nombre = quien.split()[0]
            reto = ""
            for rid, rtit in RETOS_REGULAR:
                if _re.search(r"\(\s*recupera a\s+" + _re.escape(nombre) + r"\s*\)", rtit, _re.I) or \
...
if __import__("os").environ.get("SG_VER_FRAGMENTOS"):
    for f in FRAGMENTOS: print(f["n"], f["titulo"], "· reto", f["reto"], "· semana", f["sem"], "→ pública", f["publica"])
assert len(FRAGMENTOS) >= 8, "no encuentro los fragmentos en el CRONO (%d)" % len(FRAGMENTOS)
assert sum(1 for f in FRAGMENTOS if f["reto"]) >= 8, \
    "algún fragmento se ha quedado sin su reto: " + ", ".join(f["titulo"] for f in FRAGMENTOS if not f["reto"])

FRAGMENTOS_JSON = json.dumps(FRAGMENTOS, ensure_ascii=False)
# 24-sep · LO QUE SE ABRE TRAS LA BATALLA sin ser fragmento: el desenlace («La batalla de la Ciudadela Gris»). Se abre como el
# Fragmento Prohibido —en la Nave, cuando acaba el viaje— y no se proyecta en la sesión. No cuenta como fragmento.
TRAS_BATALLA = [{"id": yt(c)["id"], "titulo": yt(c)["titulo"], "reto": "", "desenlace": True, "sem": s["sem"], "publica": s["sem"] + 1}
                for s in CRONO for c, _ in s["videos"] if c == "desenlace"]
assert TRAS_BATALLA, "el desenlace no está en el calendario (CRONO)"
TRAS_BATALLA_JSON = json.dumps(TRAS_BATALLA, ensure_ascii=False)
...
<script>window.SG_PRESENTA={_PRESENTA_JSON};window.SG_PRES_GUIA_V={_PRES_GUIA_V};window.SG_GENIALLY_CARPETA={json.dumps(GENIALLY_CARPETA)};window.SG_AVISO_RETOS_DIA={AVISO_RETOS_DIA};window.SG_SEMANAS={SEMANAS_JSON};window.SG_PLANETAS={json.dumps(PLANETAS, ensure_ascii=False)};window.SG_RETOS={json.dumps({"REGULAR": RETOS_REGULAR, "PUA": RETOS_PUA}, ensure_ascii=False)};window.SG_CAPITULOS={CAPITULOS_JSON};window.SG_CROMOS={json.dumps([list(c) for c in CROMOS], ensure_ascii=False)};window.SG_CARDV="?v={_cardv}";window.SG_IMGV="?v={hashlib.md5("".join(open(os.path.join(HERE,"assets","img","planetas",k+".png"),"rb").read().hex()[:64] for k,*_ in PLANETAS).encode()).hexdigest()[:10]}";window.SG_TOPE_SEMANA={TOPE_RETOS_SEMANA};window.SG_PER_ESCUELA={json.dumps(PER_ESCUELA)};window.SG_ENLACES={json.dumps([list(x) for x in ENLACES_EQUIPO], ensure_ascii=False)};window.SG_ACTIVIDADES={json.dumps(ACTIVIDADES, ensure_ascii=False)};window.SG_PLANTILLA_EP={json.dumps(PLANTILLA_EPORTFOLIO)};</script>
...
''' + '<script>window.SG_BADGES=' + json.dumps(NAVE_BADGES) + ';window.SG_BADGE_NAMES=' + json.dumps(BADGE_NAME, ensure_ascii=False) + ';window.SG_BATALLA=' + json.dumps(BATALLA, ensure_ascii=False) + ';window.SG_SEMANAS=' + SEMANAS_JSON + ';window.SG_SEM_RETO=' + SEM_RETO_JSON + ';window.SG_GANCHO_RETOS=' + json.dumps(GANCHO_RETOS, ensure_ascii=False) + ';window.SG_AYUDA_RETOS=' + json.dumps(_AYUDA_NAVE, ensure_ascii=False) + ';window.SG_ACTIVIDADES=' + json.dumps(_ACT_NAVE, ensure_ascii=False) + ';window.SG_PLANTILLA_EP=' + json.dumps(PLANTILLA_EPORTFOLIO) + ';window.SG_AVISO_RETOS_DIA=' + str(AVISO_RETOS_DIA) + ';window.SG_EJEMPLOS=' + json.dumps(_EJ_NAVE, ensure_ascii=False) + ';</script>' + '<script src="' + _v("assets/js/tkcomun.js") + '" defer></script>' + '<script src="' + _v("assets/js/consola.js") + '" defer></script>' + '<script src="' + _v("assets/js/tablero.js") + '" defer></script>' + '''
</div></section>
''' + FOOT
