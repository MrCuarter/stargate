...
- `recluta.html` — **La Nave del Recluta** (v3.1, 24-ago): la web del alumnado por PER (`?per=<id>`, `&embed=1`): el recluta se identifica con su CORREO una vez por dispositivo (POST `accion=quien`, sin PIN; el correo nunca va en la URL ni se lista). **Nace CERRADA (9-sep):** sin identificar no se pinta nada — ni pestañas, ni accesos a los formularios, ni tablero — y el boton de alistarse aparece solo tras buscar un correo y no encontrarlo. El motivo es concreto: el formulario de dudas es ANONIMO, asi que a la vista de cualquiera era una invitacion a ensuciar la clase. `&demo=1` abre la Nave sin correo con un recluta del tablero publico, **solo en grupos con DEMO o PRUEBA en el nombre** (la misma regla que `sembrarDemo()`), para enseñar la plataforma sin exponer a nadie real. Una vez dentro ve SU personaje (avatar+rango+xp+colección+bio); onboarding con NEBULA, planetas desbloqueados por calendario, foro unidireccional, recompensas con desbloqueo semanal y botón al panel de control Genially del PER. Motor compartido en `assets/js/calendario.js`.
